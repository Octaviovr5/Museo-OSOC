...
// 🔴 Mostrar alerta si WebGL está bloqueado en iOS
if (!checkWebGL()) {
  alert("Tu navegador no admite WebGL. Actívalo en Configuración > Safari > Avanzado > WebGL.");
}

// 🌀 INICIALIZAR PANELES 360°
let viewer, viewerone;
...
function startPanorama() {
  viewer = pannellum.viewer('panorama', {
      ...
      showControls: true,
      touchPan: true,
      orientationOnByDefault: isIOS() ? false : true // iOS tiene problemas con esto
  });
}
...
function startPanoramaLV() {
  viewerone = pannellum.viewer('panoramalvone', {
      ...
      showControls: true,
      touchPan: true,
      orientationOnByDefault: isIOS() ? false : true
  });
}

// 📌 PEDIR PERMISO DE GIROSCOPIO EN iOS
function requestGyroscopePermission() {
  if (isIOS() && typeof DeviceMotionEvent.requestPermission === 'function') {
      DeviceMotionEvent.requestPermission()
          .then(response => {
              if (response === 'granted') {
                  console.log("✅ Giroscopio activado en iOS");
                  startPanorama();
                  startPanoramaLV();
              } else {
                  alert("❌ Acceso al giroscopio denegado. Actívalo en la configuración de Safari.");
              }
          })
          .catch(error => {
              console.error("Error en la solicitud del giroscopio:", error);
          });
  } else {
      startPanorama();
      startPanoramaLV();
...
// ⏳ Cargar panoramas cuando se inicie la página
window.addEventListener("load", requestGyroscopePermission);


// ✨ ANIMACIÓN FINAL SECTION
const observer = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
      const target = entry.target;
      if (entry.isIntersecting) {
